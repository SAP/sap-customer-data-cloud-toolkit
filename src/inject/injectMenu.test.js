...
const { initMenuExtension, destroyMenuExtension, injectMenu, MENU_ELEMENT_CLASS, ADMIN_BUTTON_SELECTOR, ADMIN_BUTTON_CLASSES } = require('./injectMenu')
...
it('CDC Toolbox menu buttons injected when Admin Button exists and removed when it desappears', () => {
  ...
  injectedButtons = document.querySelectorAll(`.${MENU_ELEMENT_CLASS}`)
  expect(injectedButtons.length).toBe(0)
