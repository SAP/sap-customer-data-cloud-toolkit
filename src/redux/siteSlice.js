...
const getParentFromStructure = (structure) => {
	...
	return {
		...
		childSites: addChildsFromStructure(
			tempId,
			structure.dataCenter,
			structure.childSites,
		),
		isChildSite: false,
	}
}
	...
	reducers: {
		...
		deleteParent: (state, action) => {
			...
			state.sites = state.sites.filter(
				(site) => site.tempId !== parentSiteTempId,
			)
		},
		...
		updateParentDataCenter: (state, action) => {
			...
			parentSite.childSites.forEach(
				(childSite) => (childSite.dataCenter = sourceParent.newDataCenter),
			)
		},
		...
		deleteChild: (state, action) => {
			...
			const parentSite = getSiteById(state.sites, parentSiteTempId)
			parentSite.childSites = parentSite.childSites.filter(
				(childSite) => childSite.tempId !== childTempId,
			)
		},
