...
describe('siteDeployerCopyConfiguration test suite', () => {
  ...
  it('should update the source site when adding a second source site', () => {
    ...
    cy.get('#siteCopyConfigurationDialog').find('#addTargetSiteButton').click()
    utils.checkTargetSitesList()
  })
  ...
  it('should clear site configurations when removing the source site', () => {
    ...
    cy.wait(1000)
    utils.setConfigurationCheckBox('#siteCopyConfigurationDialog')
    cy.get('#siteCopyConfigurationDialog').find('#siteConfigurationsCard').should('be.visible')
  ...
  it('should set dataflow configuration variables on save and not on cancel', () => {
    ...
    utils.getSiteCopyConfigurationDialog().should('be.visible')
    utils.fillSourceApiKeyInput()
    cy.get('#siteCopyConfigurationDialog').find('ui5-tree').eq(8).find('ui5-checkbox').eq(0).realClick()
    cy.get('#siteCopyConfigurationDialog').find('ui5-tree').eq(8).find('ui5-tree-item-custom').shadow().find('.ui5-li-tree-toggle-box').realClick()
    cy.get('#siteCopyConfigurationDialog').find('ui5-tree').eq(8).find('ui5-tree-item-custom').find('ui5-tree-item-custom').eq(0).find('div').eq(0).find('ui5-button').click()
    cy.get('[data-cy="dataflowSettingsDialog"]').should('be.visible')
