...
describe('siteDeployerCopyConfiguration test suite', () => {
  ...
  it('should set dataflow configuration variables on save and not on cancel', () => {
    ...
    cy.get('[data-cy="siteCopyConfigurationDialog"]')
      ...
      .find('ui5-button')
      .click()
    cy.get('[data-cy="dataflowVariableInput"]').eq(0).should('have.value', 'test')
    cy.get('[data-cy="dataflowVariableInput"]').eq(0).shadow().find('[class = "ui5-input-inner"]').type('updated')
    cy.get('[data-cy="dialogMessageConfirmCancelButton"]').eq(1).realClick()
    cy.get('[data-cy="siteCopyConfigurationDialog"]')
      .find('ui5-tree')
      .eq(8)
      .find('ui5-tree-item-custom')
